...
for filename in os.listdir(source_folder):
    last_name = remove_numbers(filename.split('_')[1]).lower()
    if last_name not in ['vartan', 'bracco', 'butler', 'harmon', 'gilpin', 'radcliffe']:
        img = Image.open(os.path.join(source_folder, filename)).convert('L')
        img = img.resize((dim, dim))

        # Convert the image to a numpy array and append to the training set with the label
        img_array = np.array(img)
        #print(last_name in ['anderson', 'cattrall', 'marie', 'conn', 'delany', 'electra', 'hartley', 'hatcher', 'innes', 'louis-dreyfus', 'marcil', 'meyer', 'richter', 'elwes', 'klein', 'noth', 'dourdan', 'statham', 'smith', 'dicaprio', 'long', 'gray', 'walker', 'madden'])
        if  last_name in ['anderson', 'cattrall', 'marie', 'conn', 'delany', 'electra', 'hartley', 'hatcher', 'innes', 'louis-dreyfus', 'marcil', 'meyer']:
            label = 1
        elif last_name in ['richter', 'elwes', 'klein', 'noth', 'dourdan', 'statham', 'smith', 'dicaprio', 'long', 'gray', 'walker', 'madden']:
            label = 0
        data_full.append((img_array, label))
print(len(data_full))
